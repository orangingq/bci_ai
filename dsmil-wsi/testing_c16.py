...
if __name__ == '__main__':
    ...
    aggregator_weights = torch.load('example_aggregator_weights/c16_aggregator.pth', weights_only=True)
    milnet.load_state_dict(aggregator_weights, strict=False)
    
    state_dict_weights = torch.load(os.path.join('test-c16', 'weights', 'embedder.pth'), weights_only=True)
